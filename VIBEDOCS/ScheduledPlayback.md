...
保号任务列表和播放源选择器必须遵守隐私模式边界。普通模式只加载 `servers.private_mode = 0` 的任务与 Emby 会话；用户通过 PIN 进入隐私模式后，同时加载普通服务和隐私服务，隐私服务卡片与任务卡片显示 `Private` 标签。进入或退出隐私模式，以及修改服务的隐私属性时，都会停止当前后台任务并刷新模型，防止服务名称、任务状态或编辑上下文跨模式残留。
