...
WebDAV 文件页提供三种显示模式（切换按钮复用 `OptionSegmentedControl`，选中块以
  200ms `OutCubic` 滑移到新模式，见 `OptionSegmentedControl.md`）。为避免列表重建
  阻塞 GUI 线程导致滑移动画卡顿，切换采两段式延迟提交：点击后开关先在本地预览新分段
  并播放滑移动画，230ms 后发出 `commitRequested`；页面（`webDavListArea`）接管提交，
  把三种视图包在带 `Translate` 变换的 `webDavModeStack` 里执行方向性交接动画：旧内容
  按模式顺序 `default → video → audio` 的方向先 130ms 滑出淡出，随后在内容完全隐藏的
  时间槽内写入 `AppViewModel`（过滤模型重建与三视图切换全部发生在不可见时刻，卡顿
  不可见）；滑入不会紧跟提交启动，而是由 `modeEnterWatchTimer` 以 16ms 轮询门控揭示：
  直到目标视图 `count===0` 或 `contentItem` 已有 delegate 且再稳定 3 拍（留给布局与
  首帧绘制）后才开始 170ms 滑入淡入，避免露出尚未渲染完成的白屏再闪烁出列表；
  800ms 超时保底揭示；交接触发前若 ViewModel 被其它路径改写，
  则取消待提交并以 ViewModel 为准；交接中途再次点击时，上一次提交立即落地，动画从
  当前实际模式重新起跳，不会产生叠加动画或残留位移；
...
- 播放策略由 `webDavAudioRepeatMode` 保存：`off` 为顺序播放，末曲结束后停止；`one` 在正常 EOF 后重播当前曲目；`all` 在正常 EOF 播完末曲后回到第一首。
- 单个文件加载失败时记录错误并继续下一首。失败事件不触发单曲重播，也不会从失败的末曲回绕列表，避免服务器不可用时形成无限错误循环。
- 手动上一首/下一首在列表循环模式下可跨越队列首尾，顺序播放和单曲循环模式仍遵守队列边界。
- 音频加载、缓冲和定位状态显示在暂停/播放主按钮内部，不复用视频播放的全屏加载提示。`PlayerController` 在 `MPV_EVENT_START_FILE` 到 `MPV_EVENT_FILE_LOADED` / `MPV_EVENT_PLAYBACK_RESTART` 之间保持加载状态；`loadfile replace` 结束旧曲目的 `STOP` 事件不会提前清除新曲目的加载指示。
- QML 音乐播放器只展示当前曲目、进度、暂停、上一首、下一首、音量、循环模式和播放队列，并把用户操作转发给 C++ ViewModel，不在 QML 中计算或维护队列。传输控制与循环模式使用固定尺寸的本地绘制图标，避免平台字体差异改变图标外观。
- 音频播放器左上角返回和 Esc 只返回 WebDAV 文件页，不停止播放。播放器缩为右下角可拖动展板，并继续复用原 `MpvVideoItem` 的暂停、加载、进度和曲目状态；点击展板主体恢复完整播放器，展板上的退出按钮才会清空播放 URL、撤销代理流并停止音频。
- 完整播放器和迷你播放器通过 `PlayerController` 读取 libmpv 的 `metadata` 属性，显示音频内嵌的标题、艺术家、专辑、流派、日期和音轨编号；缺少标题或艺术家时分别回退到 WebDAV 文件名和当前服务器名。切歌发起时会先清空上一首的元数据，避免加载阶段串歌。
- `PlayerController` 根据 libmpv `track-list/N/albumart` 识别音频内嵌封面，并通过客户端 API 的 `screenshot-raw video rgba` 读取封面帧。图片缩放后只写入播放器生命周期内的临时目录，不额外向 WebDAV 发起图片请求；切歌或停止时立即清除旧封面。
- 完整音频页将标题、艺术家、专辑等元数据排列在封面左侧，封面右侧展示；迷你播放器也会用同一封面替换默认音符图标。没有封面或封面尚未加载完成时继续显示原有占位样式。
- 内嵌封面由 `PlayerController` 在 headless 音频模式下通过 `screenshot-raw` 截取图片轨道帧提取：抓取失败时先以 140ms 间隔快试 3 次，随后降频为 600ms 继续重试直到首帧真正可解码（总预算 18 次）；仅靠事件驱动的抓取点（video-reconfig / playback-restart）可能全部早于专辑封面轨道在 `track-list` 中标记，因此轨道挂载时还会直接补发一次抓取；此前“短突发失败即永久放弃”的策略会在 WebDAV 慢响应下随机导致封面不显示。
- 两种播放器中的封面容器都锁定最小、建议和最大布局尺寸。隐藏的 `Image` 只提供原始宽高，`Canvas` 通过 `loadImage` 单独加载 URL，按 `PreserveAspectCrop` 等价规则计算取景区域并只在圆角路径内绘制，确保图片可见且不会穿过圆角边界。
- 迷你播放器的进度条使用带内边距的圆角轨道，不再贴住组件底边，因此拖动展板或不同缩放比例下都不会越过外层圆角。
- 迷你播放器必须作为页面根容器（`mainPageRoot`）的直接子项而不是 `ColumnLayout` 子项
  实例化：它是 `z: 200` 的悬浮覆盖层，若处于布局行中，缩小可见后会在页面栈下方占用
  94px 布局行，导致音频列表等页面底部被截断、出现空白带。
...
- 文件明细页提供全部、未完成、已完成、失败、已取消五种等宽筛选；未完成包含等待中、传输中、自动重试等待中和已暂停状态。
...
- 网络级临时错误（连接被远端关闭、连接超时、临时网络故障等）以及 408/425/429/502/503/504 等临时 HTTP 响应会自动重试最多 3 次（总计最多 4 次尝试），重试间隔按 1 秒、2 秒、4 秒递增；任务在等待期间显示 `retrying`，日志会记录尝试次数、退避时间和错误原因。其他 HTTP 业务错误仍保留为可手动重试的失败任务。
- 第一版不支持上传断点续传或分块续传协议；暂停和自动重试都会重新打开源文件并从头发起完整 `PUT`，不会续传已发送的字节。
...
WebDAV 视频和音频播放都通过本地 `WebDavPlaybackProxy` 把远程内容交给 libmpv。视频使用窗口嵌入；音频使用不创建视频窗口、但保留音频输出的 headless libmpv 实例。音频专用实例在播放器页面因 `StackLayout` 导航变为不可见时仍继续运行，只有播放源被清空、用户显式退出或组件销毁时才停止。
