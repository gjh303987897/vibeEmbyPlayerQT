...
layouts. Jellyfin uses the trendy layout.
...
The traditional Emby home restores the standard application toolbar and page
spacing. It presents portrait continue-watching cards in a horizontal rail and
libraries in a responsive grid. Traditional cards preserve the series-name and
season/episode metadata added to the trendy home.

## Data Contract

`AppViewModel::recommendedItems` is populated from Emby's official
`GET /Users/{UserId}/Suggestions` endpoint with `IncludeItemTypes=Series`.
Jellyfin sessions and failed or empty recommendation responses fall back to
`continueItems`, so recommendation availability never blocks the home screen.
...
Layout selection is exposed by `AppViewModel::embyHomeLayout`. Switching the
layout does not trigger additional requests; both presentations reuse
`continueItems`, `libraries`, and the existing details/navigation commands.
