...
- Windows 主窗口使用 `Qt.FramelessWindowHint` 隐藏系统标题栏。非 Emby 页面继续使用原有右上角窗口控制组。进入 Emby 内容页面后，最小化、最大化/还原和关闭按钮改由右上角小型半透明圆角悬浮控制组提供；鼠标在窗口顶部停留 0.5 秒后控制组从上沿滑入，不参与页面布局，离开后短暂延迟收起。
...
- 应用工具栏通过不参与布局的 `DragHandler` 识别拖动手势，再由 `startSystemMove()` 调用 `QWindow::startSystemMove()`。Emby 内容子页面额外提供顶部拖动热区，现代播放器的视频顶部也提供独立拖动热区；这些热区避开窗口控制和业务按钮，普通点击仍由业务按钮处理。八个边缘/角落通过 `startSystemResize(Qt::Edges)` 交回系统处理，因此保留 Windows 吸附、跨屏拖动和原生缩放约束。
- 双击应用工具栏的页面标题区域切换最大化与还原；最大化和全屏时禁用边缘缩放热区。悬浮窗口控制组在全屏时隐藏。
